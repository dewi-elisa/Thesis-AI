...
In the encoder part, it first embeds the tokens of the subsequence.
It then encodes the embedding using a bi-directional LSTM. 

In the decoder part, the full sentence is decoded. 
It therefore embeds the already decoded sequence (or just the <eos> symbol if there is none) into a 300-dimensional vector and concatenates the last hidden state of the encoder* to it. 
